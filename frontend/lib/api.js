...
function fallbackRegion(code = "om", locale = "en") {
  ...
  const regionMap = {
    om: {
      ...
      contact_email: "sales@enfant-me.com",
      address: "IFZA Business Park - Building A02 - Dubai Silicon Oasis - Industrial Area - Dubai - United Arab Emirates",
      is_default: true,
    },
    ae: {
      code: "ae",
...
function fallbackNavigation(locale, region) {
  ...
  const settings = {
    ...
    announcement: "Summer Sale - Upto 40% Off On All Products",
    footer_about: isAr
      ? "منتجات عناية طبيعية ولطيفة للأطفال."
      : "Natural, gentle baby-care essentials for everyday routines.",
    newsletter_title: "Don’t Miss Out Any Sale",
    newsletter_subtitle: "Subscribe to receive discount updates at very first.",
    discount_popup: {
      enabled: true,
...
function fallbackHomePage(locale) {
  ...
  return {
    ...
    newsletter: {
      title: "Don’t Miss Out Any Sale",
      subtitle: "Subscribe to receive discount updates at very first.",
